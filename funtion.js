...
function cutstr(str, len) {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (icount < len - 1) { //小于设定的len值就执行

            temp = str.substr(i, 1);

            if (patrn.exec(temp) == null) { //匹配英文就加1
    ...
    return strre + "..." //r返回时加上省略号
}

//替换全部
String.prototype.replaceAll = function(s1, s2) { //s1为要匹配的对象或字符串，s2为要替换后的新对象或字符串
    return this.replace(new RegExp(s1, "gm"), s2); //全局多行匹配
}
